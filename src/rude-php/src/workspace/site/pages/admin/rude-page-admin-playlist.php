		...
		$q =
			'
				...
				FROM
					playlist_items
				LEFT JOIN
					songs ON songs.id = playlist_items.song_id
				WHERE
					playlist_items.playlist_id = ' . (int) $playlist_id . '
		';
		...
		switch ($action)
		{
			case 'add':
				...
				if ($song_ids)
				{
					...
					{
						$q = new query_select('playlist_items');
						...
						{
							playlist_items::add($playlist_id, $song_id);
						}
					}
			...
			case 'remove':
				...
				if ($song_ids)
				{
					$q = new query_delete('playlist_items');
				...
				foreach ($playlists as $playlist)
				{
					...
					<tr>
						...
						<td class="nowrap">
							...
							<a class="inline-block teal" href="?page=admin&task=playlist&task2=edit&playlist-id=<?= $playlist->id ?>">
