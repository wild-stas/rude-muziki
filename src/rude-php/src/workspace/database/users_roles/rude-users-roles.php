...
if (!defined('RUDE_DATABASE_TABLE_USERS_ROLES'))             { define('RUDE_DATABASE_TABLE_USERS_ROLES',             'users_roles'); }
if (!defined('RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY')) { define('RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY', 'id'); }

class users_roles
{
	public static function get($id = null, $limit = null, $offset = null)
	{
		$q = new query_select(RUDE_DATABASE_TABLE_USERS_ROLES);
		$q->limit($limit, $offset);

		if ($id !== null)
		{
			$q->where(RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY, $id);
			$q->query();
	...
	{
		$q = new query_select(RUDE_DATABASE_TABLE_USERS_ROLES);
		$q->order_by(RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY, 'DESC');
		$q->limit($n);
		$q->query();
	...
	{
		$q = new query_select(RUDE_DATABASE_TABLE_USERS_ROLES);
		$q->order_by(RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY, 'ASC');
		$q->limit($n);
		$q->query();
	...
	{
		$q = new query_insert(RUDE_DATABASE_TABLE_USERS_ROLES);
	...
	{
		$q = new query_update(RUDE_DATABASE_TABLE_USERS_ROLES);
	...
	{
		$q = new query_delete(RUDE_DATABASE_TABLE_USERS_ROLES);
		$q->where(RUDE_DATABASE_TABLE_USERS_ROLES_PRIMARY_KEY, $id);
		$q->limit($limit, $offset);
		$q->query();
	...
	{
		$database = database();
		$database->query('SELECT COUNT(*) as count FROM ' . RUDE_DATABASE_TABLE_USERS_ROLES);
